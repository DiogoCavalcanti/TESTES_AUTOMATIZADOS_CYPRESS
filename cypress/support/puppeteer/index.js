    ...
    setup({
      ...
      onMessage: {
        async switchTabAndGetContent (browser) {
          ...
          const page = await pageRetrier(browser, 'https://desenvolvimento.pje.csjt.jus.br/pjekz/processo/369117/tarefa/63/transicao')
          await page.bringToFront()
          
          await page.waitForSelector('[aria-label="Análise"] > .mat-button-wrapper > .texto-botao-app')

          await page.click('[aria-label="Análise"] > .mat-button-wrapper > .texto-botao-app')

          await page.waitForSelector('[aria-label="Encaminhar ao CEJUSC"]')
          await page.click('[aria-label="Encaminhar ao CEJUSC"]')

          await page.waitForSelector('.opcoes > .mat-focus-indicator')
          await page.click('.opcoes > .mat-focus-indicator')
